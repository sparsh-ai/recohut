...
class dbtFlow(FlowSpec):
    ...
    def run_transformation(self):
        ...
        if is_cloud:
            ...
            dbt_cmd = "dbt run --vars '{SF_SCHEMA: %s, SF_TABLE: %s}' --profiles-dir ."  % (os.getenv('SF_SCHEMA'), os.getenv('SF_TABLE'))
            # debug
            print("dir is {}, dbt command is: {}".format(my_dir, dbt_cmd))
            # run dbt from python
            process = subprocess.Popen(dbt_cmd, cwd=os.path.join(my_dir, 'dbt/'), shell=True)
            process.communicate()

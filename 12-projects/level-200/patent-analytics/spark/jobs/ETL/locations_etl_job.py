...
def main():
    spark, log, config = start_spark(
        app_name="locations_etl_job", files=["../configs/etl_config.json"]
    )

    log.warn("Locations ETL Job is up and running")
    ...
    log.warn("Locations ETL Job is finished")
    spark.stop()
    return None
...
def transform_data(data):
    transformed_data = (
        data.select("location_id", "country", "continent")
        .distinct()

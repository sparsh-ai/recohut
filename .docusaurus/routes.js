import React from 'react';
import ComponentCreator from '@docusaurus/ComponentCreator';

export default [
  {
    path: '/__docusaurus/debug',
    component: ComponentCreator('/__docusaurus/debug', 'b27'),
    exact: true
  },
  {
    path: '/__docusaurus/debug/config',
    component: ComponentCreator('/__docusaurus/debug/config', '910'),
    exact: true
  },
  {
    path: '/__docusaurus/debug/content',
    component: ComponentCreator('/__docusaurus/debug/content', '82f'),
    exact: true
  },
  {
    path: '/__docusaurus/debug/globalData',
    component: ComponentCreator('/__docusaurus/debug/globalData', 'aa3'),
    exact: true
  },
  {
    path: '/__docusaurus/debug/metadata',
    component: ComponentCreator('/__docusaurus/debug/metadata', '716'),
    exact: true
  },
  {
    path: '/__docusaurus/debug/registry',
    component: ComponentCreator('/__docusaurus/debug/registry', 'b62'),
    exact: true
  },
  {
    path: '/__docusaurus/debug/routes',
    component: ComponentCreator('/__docusaurus/debug/routes', '480'),
    exact: true
  },
  {
    path: '/blog',
    component: ComponentCreator('/blog', 'd3a'),
    exact: true
  },
  {
    path: '/blog/2020/01/19/mobilenet-ssd-caffe-pre-trained-model',
    component: ComponentCreator('/blog/2020/01/19/mobilenet-ssd-caffe-pre-trained-model', '1ea'),
    exact: true
  },
  {
    path: '/blog/2021/01/23/object-detection-with-yolo3',
    component: ComponentCreator('/blog/2021/01/23/object-detection-with-yolo3', '8c8'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/clinical-decision-making',
    component: ComponentCreator('/blog/2021/10/01/clinical-decision-making', '47f'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/detectron-2',
    component: ComponentCreator('/blog/2021/10/01/detectron-2', '12b'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/distributed-training-of-recommender-systems',
    component: ComponentCreator('/blog/2021/10/01/distributed-training-of-recommender-systems', '031'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/document-recommendation',
    component: ComponentCreator('/blog/2021/10/01/document-recommendation', '5c2'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/fake-voice-detection',
    component: ComponentCreator('/blog/2021/10/01/fake-voice-detection', 'd88'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/image-similarity-system',
    component: ComponentCreator('/blog/2021/10/01/image-similarity-system', '8a4'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/insurance-personalization',
    component: ComponentCreator('/blog/2021/10/01/insurance-personalization', 'a92'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/name-&-address-parsing',
    component: ComponentCreator('/blog/2021/10/01/name-&-address-parsing', '208'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/object-detection-hands-on-exercises',
    component: ComponentCreator('/blog/2021/10/01/object-detection-hands-on-exercises', 'a47'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/object-detection-with-opencv',
    component: ComponentCreator('/blog/2021/10/01/object-detection-with-opencv', '662'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/object-detection-with-yolo3',
    component: ComponentCreator('/blog/2021/10/01/object-detection-with-yolo3', '40c'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/ocr-experiments',
    component: ComponentCreator('/blog/2021/10/01/ocr-experiments', 'af7'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/pdf-to-wordcloud-via-mail',
    component: ComponentCreator('/blog/2021/10/01/pdf-to-wordcloud-via-mail', '51c'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/personalized-unexpectedness-in-recommender-systems',
    component: ComponentCreator('/blog/2021/10/01/personalized-unexpectedness-in-recommender-systems', '94f'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/predicting-electronics-resale-price',
    component: ComponentCreator('/blog/2021/10/01/predicting-electronics-resale-price', 'caf'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/real-time-news-personalization-with-flink',
    component: ComponentCreator('/blog/2021/10/01/real-time-news-personalization-with-flink', 'cd9'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/semantic-similarity',
    component: ComponentCreator('/blog/2021/10/01/semantic-similarity', 'af8'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/short-video-background-music-recommender',
    component: ComponentCreator('/blog/2021/10/01/short-video-background-music-recommender', '4a1'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/the-progression-of-analytics-in-enterprises',
    component: ComponentCreator('/blog/2021/10/01/the-progression-of-analytics-in-enterprises', 'dc3'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/tools-for-building-recommender-systems',
    component: ComponentCreator('/blog/2021/10/01/tools-for-building-recommender-systems', '5f6'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/vehicle-suggestions',
    component: ComponentCreator('/blog/2021/10/01/vehicle-suggestions', '1fa'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/web-scraping-using-scrapy-bs4-and-selenium',
    component: ComponentCreator('/blog/2021/10/01/web-scraping-using-scrapy-bs4-and-selenium', '3c8'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/web-scraping-with-gazpacho',
    component: ComponentCreator('/blog/2021/10/01/web-scraping-with-gazpacho', 'df7'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/wellness-tracker-chatbot',
    component: ComponentCreator('/blog/2021/10/01/wellness-tracker-chatbot', '911'),
    exact: true
  },
  {
    path: '/blog/2021/10/01/what-is-livestream-ecommerce',
    component: ComponentCreator('/blog/2021/10/01/what-is-livestream-ecommerce', '591'),
    exact: true
  },
  {
    path: '/blog/2022/11/16/opening-material',
    component: ComponentCreator('/blog/2022/11/16/opening-material', 'b4d'),
    exact: true
  },
  {
    path: '/blog/2022/12/12/the-complete-python-course-2022',
    component: ComponentCreator('/blog/2022/12/12/the-complete-python-course-2022', 'ae3'),
    exact: true
  },
  {
    path: '/blog/2022/12/13/sql-to-snowflake-schema-conversion',
    component: ComponentCreator('/blog/2022/12/13/sql-to-snowflake-schema-conversion', '4ee'),
    exact: true
  },
  {
    path: '/blog/2022/12/24/ab-mab-tests',
    component: ComponentCreator('/blog/2022/12/24/ab-mab-tests', '563'),
    exact: true
  },
  {
    path: '/blog/archive',
    component: ComponentCreator('/blog/archive', '8d8'),
    exact: true
  },
  {
    path: '/blog/page/2',
    component: ComponentCreator('/blog/page/2', 'a5d'),
    exact: true
  },
  {
    path: '/blog/page/3',
    component: ComponentCreator('/blog/page/3', 'ada'),
    exact: true
  },
  {
    path: '/blog/page/4',
    component: ComponentCreator('/blog/page/4', 'e75'),
    exact: true
  },
  {
    path: '/blog/tags',
    component: ComponentCreator('/blog/tags', '0a1'),
    exact: true
  },
  {
    path: '/blog/tags/ab',
    component: ComponentCreator('/blog/tags/ab', 'f21'),
    exact: true
  },
  {
    path: '/blog/tags/app',
    component: ComponentCreator('/blog/tags/app', '891'),
    exact: true
  },
  {
    path: '/blog/tags/audio',
    component: ComponentCreator('/blog/tags/audio', 'dbd'),
    exact: true
  },
  {
    path: '/blog/tags/aws-beanstalk',
    component: ComponentCreator('/blog/tags/aws-beanstalk', '449'),
    exact: true
  },
  {
    path: '/blog/tags/chatbot',
    component: ComponentCreator('/blog/tags/chatbot', 'a73'),
    exact: true
  },
  {
    path: '/blog/tags/classification',
    component: ComponentCreator('/blog/tags/classification', '12e'),
    exact: true
  },
  {
    path: '/blog/tags/course',
    component: ComponentCreator('/blog/tags/course', 'b3c'),
    exact: true
  },
  {
    path: '/blog/tags/dataengineering',
    component: ComponentCreator('/blog/tags/dataengineering', 'fc5'),
    exact: true
  },
  {
    path: '/blog/tags/deepfake',
    component: ComponentCreator('/blog/tags/deepfake', '826'),
    exact: true
  },
  {
    path: '/blog/tags/distributed',
    component: ComponentCreator('/blog/tags/distributed', '76e'),
    exact: true
  },
  {
    path: '/blog/tags/docusaurus',
    component: ComponentCreator('/blog/tags/docusaurus', '42b'),
    exact: true
  },
  {
    path: '/blog/tags/flask',
    component: ComponentCreator('/blog/tags/flask', 'f55'),
    exact: true
  },
  {
    path: '/blog/tags/healthcare',
    component: ComponentCreator('/blog/tags/healthcare', 'da5'),
    exact: true
  },
  {
    path: '/blog/tags/insight',
    component: ComponentCreator('/blog/tags/insight', '002'),
    exact: true
  },
  {
    path: '/blog/tags/insurance',
    component: ComponentCreator('/blog/tags/insurance', '36c'),
    exact: true
  },
  {
    path: '/blog/tags/mab',
    component: ComponentCreator('/blog/tags/mab', 'b5b'),
    exact: true
  },
  {
    path: '/blog/tags/ner',
    component: ComponentCreator('/blog/tags/ner', '5ba'),
    exact: true
  },
  {
    path: '/blog/tags/nlp',
    component: ComponentCreator('/blog/tags/nlp', '706'),
    exact: true
  },
  {
    path: '/blog/tags/object-detection',
    component: ComponentCreator('/blog/tags/object-detection', 'b95'),
    exact: true
  },
  {
    path: '/blog/tags/ocr',
    component: ComponentCreator('/blog/tags/ocr', 'e9a'),
    exact: true
  },
  {
    path: '/blog/tags/opencv',
    component: ComponentCreator('/blog/tags/opencv', '2d3'),
    exact: true
  },
  {
    path: '/blog/tags/packtpublishing',
    component: ComponentCreator('/blog/tags/packtpublishing', '571'),
    exact: true
  },
  {
    path: '/blog/tags/personalization',
    component: ComponentCreator('/blog/tags/personalization', '5de'),
    exact: true
  },
  {
    path: '/blog/tags/python',
    component: ComponentCreator('/blog/tags/python', '913'),
    exact: true
  },
  {
    path: '/blog/tags/realtime',
    component: ComponentCreator('/blog/tags/realtime', '6f4'),
    exact: true
  },
  {
    path: '/blog/tags/recohut',
    component: ComponentCreator('/blog/tags/recohut', '0aa'),
    exact: true
  },
  {
    path: '/blog/tags/recsys',
    component: ComponentCreator('/blog/tags/recsys', '584'),
    exact: true
  },
  {
    path: '/blog/tags/regression',
    component: ComponentCreator('/blog/tags/regression', '6ee'),
    exact: true
  },
  {
    path: '/blog/tags/scraping',
    component: ComponentCreator('/blog/tags/scraping', '7f2'),
    exact: true
  },
  {
    path: '/blog/tags/similarity',
    component: ComponentCreator('/blog/tags/similarity', 'b52'),
    exact: true
  },
  {
    path: '/blog/tags/snowflake',
    component: ComponentCreator('/blog/tags/snowflake', 'b07'),
    exact: true
  },
  {
    path: '/blog/tags/streamlit',
    component: ComponentCreator('/blog/tags/streamlit', '867'),
    exact: true
  },
  {
    path: '/blog/tags/tool',
    component: ComponentCreator('/blog/tags/tool', '6d9'),
    exact: true
  },
  {
    path: '/blog/tags/trend',
    component: ComponentCreator('/blog/tags/trend', 'c94'),
    exact: true
  },
  {
    path: '/blog/tags/vision',
    component: ComponentCreator('/blog/tags/vision', '9e4'),
    exact: true
  },
  {
    path: '/docs/tags',
    component: ComponentCreator('/docs/tags', '349'),
    exact: true
  },
  {
    path: '/docs/tags/cassandra',
    component: ComponentCreator('/docs/tags/cassandra', '77d'),
    exact: true
  },
  {
    path: '/docs/tags/cql',
    component: ComponentCreator('/docs/tags/cql', '100'),
    exact: true
  },
  {
    path: '/docs/tags/keyspaces',
    component: ComponentCreator('/docs/tags/keyspaces', '6a5'),
    exact: true
  },
  {
    path: '/docs/tags/nlp',
    component: ComponentCreator('/docs/tags/nlp', '849'),
    exact: true
  },
  {
    path: '/markdown-page',
    component: ComponentCreator('/markdown-page', '4a3'),
    exact: true
  },
  {
    path: '/search',
    component: ComponentCreator('/search', '5c9'),
    exact: true
  },
  {
    path: '/docs',
    component: ComponentCreator('/docs', '7f5'),
    routes: [
      {
        path: '/docs/a1-interviewprep',
        component: ComponentCreator('/docs/a1-interviewprep', '64f'),
        exact: true
      },
      {
        path: '/docs/a1-interviewprep/algorithm-problems',
        component: ComponentCreator('/docs/a1-interviewprep/algorithm-problems', 'd8f'),
        exact: true
      },
      {
        path: '/docs/a1-interviewprep/azure-data-engineering',
        component: ComponentCreator('/docs/a1-interviewprep/azure-data-engineering', '423'),
        exact: true
      },
      {
        path: '/docs/a1-interviewprep/basic-technical-questions',
        component: ComponentCreator('/docs/a1-interviewprep/basic-technical-questions', '5bd'),
        exact: true
      },
      {
        path: '/docs/a1-interviewprep/behavioral-interview-questions',
        component: ComponentCreator('/docs/a1-interviewprep/behavioral-interview-questions', '636'),
        exact: true
      },
      {
        path: '/docs/a1-interviewprep/behavioral-questions',
        component: ComponentCreator('/docs/a1-interviewprep/behavioral-questions', '1da'),
        exact: true
      },
      {
        path: '/docs/a1-interviewprep/resume-buildup',
        component: ComponentCreator('/docs/a1-interviewprep/resume-buildup', '358'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies',
        component: ComponentCreator('/docs/a3-casestudies', '7b2'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/99group',
        component: ComponentCreator('/docs/a3-casestudies/99group', 'ce7'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/airbnb',
        component: ComponentCreator('/docs/a3-casestudies/airbnb', 'b6f'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/amazon',
        component: ComponentCreator('/docs/a3-casestudies/amazon', '19e'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/bookingdotcom',
        component: ComponentCreator('/docs/a3-casestudies/bookingdotcom', 'be8'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/expedia',
        component: ComponentCreator('/docs/a3-casestudies/expedia', 'd22'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/fair',
        component: ComponentCreator('/docs/a3-casestudies/fair', '8c0'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/harmony',
        component: ComponentCreator('/docs/a3-casestudies/harmony', '8d6'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/helpshift',
        component: ComponentCreator('/docs/a3-casestudies/helpshift', '3e9'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/hometogo',
        component: ComponentCreator('/docs/a3-casestudies/hometogo', '4fe'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/intuit',
        component: ComponentCreator('/docs/a3-casestudies/intuit', '4f4'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/linkedin',
        component: ComponentCreator('/docs/a3-casestudies/linkedin', '4a5'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/myntra',
        component: ComponentCreator('/docs/a3-casestudies/myntra', '1a1'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/outfit7',
        component: ComponentCreator('/docs/a3-casestudies/outfit7', 'e48'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/panoramic',
        component: ComponentCreator('/docs/a3-casestudies/panoramic', 'ef7'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/plexure',
        component: ComponentCreator('/docs/a3-casestudies/plexure', 'bc0'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/spotify',
        component: ComponentCreator('/docs/a3-casestudies/spotify', '520'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/starbucks',
        component: ComponentCreator('/docs/a3-casestudies/starbucks', '372'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/trivago',
        component: ComponentCreator('/docs/a3-casestudies/trivago', 'f75'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/twilio',
        component: ComponentCreator('/docs/a3-casestudies/twilio', '9e2'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/twitter',
        component: ComponentCreator('/docs/a3-casestudies/twitter', '352'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/uber',
        component: ComponentCreator('/docs/a3-casestudies/uber', '0e8'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/video-stream',
        component: ComponentCreator('/docs/a3-casestudies/video-stream', 'ef3'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/webshoes',
        component: ComponentCreator('/docs/a3-casestudies/webshoes', 'eaf'),
        exact: true
      },
      {
        path: '/docs/a3-casestudies/woot',
        component: ComponentCreator('/docs/a3-casestudies/woot', 'ce3'),
        exact: true
      },
      {
        path: '/docs/assignments',
        component: ComponentCreator('/docs/assignments', '7ca'),
        exact: true
      },
      {
        path: '/docs/assignments/bellabeat-analysis',
        component: ComponentCreator('/docs/assignments/bellabeat-analysis', 'f12'),
        exact: true
      },
      {
        path: '/docs/assignments/cyclistic-analysis',
        component: ComponentCreator('/docs/assignments/cyclistic-analysis', '92b'),
        exact: true
      },
      {
        path: '/docs/assignments/florida-panthers',
        component: ComponentCreator('/docs/assignments/florida-panthers', '396'),
        exact: true
      },
      {
        path: '/docs/assignments/healthverity',
        component: ComponentCreator('/docs/assignments/healthverity', 'bcc'),
        exact: true
      },
      {
        path: '/docs/assignments/klodars-datalake-design',
        component: ComponentCreator('/docs/assignments/klodars-datalake-design', 'f96'),
        exact: true
      },
      {
        path: '/docs/assignments/logsense',
        component: ComponentCreator('/docs/assignments/logsense', 'af8'),
        exact: true
      },
      {
        path: '/docs/assignments/mistplay-takehome',
        component: ComponentCreator('/docs/assignments/mistplay-takehome', 'a73'),
        exact: true
      },
      {
        path: '/docs/assignments/mysql-s3-incremental',
        component: ComponentCreator('/docs/assignments/mysql-s3-incremental', '6d0'),
        exact: true
      },
      {
        path: '/docs/assignments/olx-python',
        component: ComponentCreator('/docs/assignments/olx-python', 'ebc'),
        exact: true
      },
      {
        path: '/docs/assignments/sakila',
        component: ComponentCreator('/docs/assignments/sakila', '166'),
        exact: true
      },
      {
        path: '/docs/assignments/spiff-takehome',
        component: ComponentCreator('/docs/assignments/spiff-takehome', '47d'),
        exact: true
      },
      {
        path: '/docs/assignments/spotify-extract-load',
        component: ComponentCreator('/docs/assignments/spotify-extract-load', 'beb'),
        exact: true
      },
      {
        path: '/docs/assignments/ternary',
        component: ComponentCreator('/docs/assignments/ternary', '363'),
        exact: true
      },
      {
        path: '/docs/assignments/twitter-s3',
        component: ComponentCreator('/docs/assignments/twitter-s3', '26f'),
        exact: true
      },
      {
        path: '/docs/b3-misc/explore-further',
        component: ComponentCreator('/docs/b3-misc/explore-further', '4eb'),
        exact: true
      },
      {
        path: '/docs/b3-misc/extras',
        component: ComponentCreator('/docs/b3-misc/extras', '6b3'),
        exact: true
      },
      {
        path: '/docs/b3-misc/readme_archive',
        component: ComponentCreator('/docs/b3-misc/readme_archive', 'cd6'),
        exact: true
      },
      {
        path: '/docs/b3-misc/resources',
        component: ComponentCreator('/docs/b3-misc/resources', '93c'),
        exact: true
      },
      {
        path: '/docs/capstones',
        component: ComponentCreator('/docs/capstones', '82f'),
        exact: true
      },
      {
        path: '/docs/capstones/acled',
        component: ComponentCreator('/docs/capstones/acled', 'ad3'),
        exact: true
      },
      {
        path: '/docs/capstones/citibike-trip-histories',
        component: ComponentCreator('/docs/capstones/citibike-trip-histories', 'f1d'),
        exact: true
      },
      {
        path: '/docs/capstones/city-pollution',
        component: ComponentCreator('/docs/capstones/city-pollution', 'acc'),
        exact: true
      },
      {
        path: '/docs/capstones/city-traffic-drone',
        component: ComponentCreator('/docs/capstones/city-traffic-drone', '6c6'),
        exact: true
      },
      {
        path: '/docs/capstones/climate',
        component: ComponentCreator('/docs/capstones/climate', 'f25'),
        exact: true
      },
      {
        path: '/docs/capstones/cloudmaze',
        component: ComponentCreator('/docs/capstones/cloudmaze', 'afb'),
        exact: true
      },
      {
        path: '/docs/capstones/dbt-redshift',
        component: ComponentCreator('/docs/capstones/dbt-redshift', '845'),
        exact: true
      },
      {
        path: '/docs/capstones/digitalskola',
        component: ComponentCreator('/docs/capstones/digitalskola', '82d'),
        exact: true
      },
      {
        path: '/docs/capstones/disaster-response',
        component: ComponentCreator('/docs/capstones/disaster-response', '5e9'),
        exact: true
      },
      {
        path: '/docs/capstones/funflix',
        component: ComponentCreator('/docs/capstones/funflix', 'fa5'),
        exact: true
      },
      {
        path: '/docs/capstones/hmc',
        component: ComponentCreator('/docs/capstones/hmc', '101'),
        exact: true
      },
      {
        path: '/docs/capstones/kinesis-flink-beam',
        component: ComponentCreator('/docs/capstones/kinesis-flink-beam', '404'),
        exact: true
      },
      {
        path: '/docs/capstones/kinesis-flink-etl',
        component: ComponentCreator('/docs/capstones/kinesis-flink-etl', '283'),
        exact: true
      },
      {
        path: '/docs/capstones/kortex',
        component: ComponentCreator('/docs/capstones/kortex', '55d'),
        exact: true
      },
      {
        path: '/docs/capstones/lufthansa',
        component: ComponentCreator('/docs/capstones/lufthansa', 'b01'),
        exact: true
      },
      {
        path: '/docs/capstones/movie-sentiment',
        component: ComponentCreator('/docs/capstones/movie-sentiment', '54c'),
        exact: true
      },
      {
        path: '/docs/capstones/multi-touch-attribution',
        component: ComponentCreator('/docs/capstones/multi-touch-attribution', 'b84'),
        exact: true
      },
      {
        path: '/docs/capstones/other/covid19-datalake',
        component: ComponentCreator('/docs/capstones/other/covid19-datalake', '235'),
        exact: true
      },
      {
        path: '/docs/capstones/other/data-ingestion-pulumi-lambda-snowflake',
        component: ComponentCreator('/docs/capstones/other/data-ingestion-pulumi-lambda-snowflake', '11b'),
        exact: true
      },
      {
        path: '/docs/capstones/other/equitize',
        component: ComponentCreator('/docs/capstones/other/equitize', '4c6'),
        exact: true
      },
      {
        path: '/docs/capstones/other/fireflow',
        component: ComponentCreator('/docs/capstones/other/fireflow', '23a'),
        exact: true
      },
      {
        path: '/docs/capstones/other/fireflow/airflow/notes',
        component: ComponentCreator('/docs/capstones/other/fireflow/airflow/notes', '473'),
        exact: true
      },
      {
        path: '/docs/capstones/other/loop-analytics',
        component: ComponentCreator('/docs/capstones/other/loop-analytics', '85e'),
        exact: true
      },
      {
        path: '/docs/capstones/other/patent-analytics',
        component: ComponentCreator('/docs/capstones/other/patent-analytics', 'cf6'),
        exact: true
      },
      {
        path: '/docs/capstones/other/patent-analytics/REPORT',
        component: ComponentCreator('/docs/capstones/other/patent-analytics/REPORT', 'ace'),
        exact: true
      },
      {
        path: '/docs/capstones/other/patent-analytics/SETUP',
        component: ComponentCreator('/docs/capstones/other/patent-analytics/SETUP', 'dfc'),
        exact: true
      },
      {
        path: '/docs/capstones/other/patent-analytics/spark',
        component: ComponentCreator('/docs/capstones/other/patent-analytics/spark', '607'),
        exact: true
      },
      {
        path: '/docs/capstones/other/reddit-kafka-sparkstream-bigquery',
        component: ComponentCreator('/docs/capstones/other/reddit-kafka-sparkstream-bigquery', 'af8'),
        exact: true
      },
      {
        path: '/docs/capstones/other/resale-price-realtime',
        component: ComponentCreator('/docs/capstones/other/resale-price-realtime', 'cd4'),
        exact: true
      },
      {
        path: '/docs/capstones/other/sqs-postgres-etl',
        component: ComponentCreator('/docs/capstones/other/sqs-postgres-etl', 'c90'),
        exact: true
      },
      {
        path: '/docs/capstones/other/star-jeans-etl',
        component: ComponentCreator('/docs/capstones/other/star-jeans-etl', '1ad'),
        exact: true
      },
      {
        path: '/docs/capstones/other/taxi-fare-prediction',
        component: ComponentCreator('/docs/capstones/other/taxi-fare-prediction', '860'),
        exact: true
      },
      {
        path: '/docs/capstones/other/text-speech-data-pipeline',
        component: ComponentCreator('/docs/capstones/other/text-speech-data-pipeline', 'fed'),
        exact: true
      },
      {
        path: '/docs/capstones/other/topic-modeling-pipeline',
        component: ComponentCreator('/docs/capstones/other/topic-modeling-pipeline', '2ec'),
        exact: true
      },
      {
        path: '/docs/capstones/other/topic-modeling-pipeline/v2/scoping',
        component: ComponentCreator('/docs/capstones/other/topic-modeling-pipeline/v2/scoping', 'e8e'),
        exact: true
      },
      {
        path: '/docs/capstones/other/twitter-sentiment-realtime',
        component: ComponentCreator('/docs/capstones/other/twitter-sentiment-realtime', '68e'),
        exact: true
      },
      {
        path: '/docs/capstones/other/twitter-snowpipe',
        component: ComponentCreator('/docs/capstones/other/twitter-snowpipe', '244'),
        exact: true
      },
      {
        path: '/docs/capstones/other/us-job-vacancies',
        component: ComponentCreator('/docs/capstones/other/us-job-vacancies', '079'),
        exact: true
      },
      {
        path: '/docs/capstones/other/youtube-insights',
        component: ComponentCreator('/docs/capstones/other/youtube-insights', 'd9b'),
        exact: true
      },
      {
        path: '/docs/capstones/recofront',
        component: ComponentCreator('/docs/capstones/recofront', '7d1'),
        exact: true
      },
      {
        path: '/docs/capstones/reddit',
        component: ComponentCreator('/docs/capstones/reddit', 'c2a'),
        exact: true
      },
      {
        path: '/docs/capstones/redshield',
        component: ComponentCreator('/docs/capstones/redshield', '6a6'),
        exact: true
      },
      {
        path: '/docs/capstones/robust-data-pipeline',
        component: ComponentCreator('/docs/capstones/robust-data-pipeline', 'd9b'),
        exact: true
      },
      {
        path: '/docs/capstones/ServerlessStreamingApp',
        component: ComponentCreator('/docs/capstones/ServerlessStreamingApp', '883'),
        exact: true
      },
      {
        path: '/docs/capstones/smartcity/analytics/flink-cdk',
        component: ComponentCreator('/docs/capstones/smartcity/analytics/flink-cdk', 'b5d'),
        exact: true
      },
      {
        path: '/docs/capstones/smartcity/analytics/producer-app',
        component: ComponentCreator('/docs/capstones/smartcity/analytics/producer-app', 'bc3'),
        exact: true
      },
      {
        path: '/docs/capstones/smartcity/analytics/producer-cdk',
        component: ComponentCreator('/docs/capstones/smartcity/analytics/producer-cdk', 'f71'),
        exact: true
      },
      {
        path: '/docs/capstones/smartcity/dataStream',
        component: ComponentCreator('/docs/capstones/smartcity/dataStream', '1f0'),
        exact: true
      },
      {
        path: '/docs/capstones/smartcity/firehose',
        component: ComponentCreator('/docs/capstones/smartcity/firehose', 'dc3'),
        exact: true
      },
      {
        path: '/docs/capstones/sparkify',
        component: ComponentCreator('/docs/capstones/sparkify', 'ea8'),
        exact: true
      },
      {
        path: '/docs/capstones/spotify',
        component: ComponentCreator('/docs/capstones/spotify', 'afe'),
        exact: true
      },
      {
        path: '/docs/capstones/twitter-sentiment-glue',
        component: ComponentCreator('/docs/capstones/twitter-sentiment-glue', '18b'),
        exact: true
      },
      {
        path: '/docs/capstones/us-immigration',
        component: ComponentCreator('/docs/capstones/us-immigration', '9d3'),
        exact: true
      },
      {
        path: '/docs/data-modeling/3nf-data-modeling',
        component: ComponentCreator('/docs/data-modeling/3nf-data-modeling', '4be'),
        exact: true
      },
      {
        path: '/docs/data-modeling/cap-theorem',
        component: ComponentCreator('/docs/data-modeling/cap-theorem', '2e6'),
        exact: true
      },
      {
        path: '/docs/data-modeling/data-modeling-stages',
        component: ComponentCreator('/docs/data-modeling/data-modeling-stages', 'ac1'),
        exact: true
      },
      {
        path: '/docs/data-modeling/data-modeling-steps',
        component: ComponentCreator('/docs/data-modeling/data-modeling-steps', 'cba'),
        exact: true
      },
      {
        path: '/docs/data-modeling/data-vault-modeling',
        component: ComponentCreator('/docs/data-modeling/data-vault-modeling', 'bb1'),
        exact: true
      },
      {
        path: '/docs/data-modeling/data-warehousing',
        component: ComponentCreator('/docs/data-modeling/data-warehousing', '7e9'),
        exact: true
      },
      {
        path: '/docs/data-modeling/designing-incremental-loading',
        component: ComponentCreator('/docs/data-modeling/designing-incremental-loading', 'd1d'),
        exact: true
      },
      {
        path: '/docs/data-modeling/designing-scd',
        component: ComponentCreator('/docs/data-modeling/designing-scd', '552'),
        exact: true
      },
      {
        path: '/docs/data-modeling/dimensional-modeling',
        component: ComponentCreator('/docs/data-modeling/dimensional-modeling', 'a76'),
        exact: true
      },
      {
        path: '/docs/data-modeling/inmon-vs-kimball',
        component: ComponentCreator('/docs/data-modeling/inmon-vs-kimball', '96c'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-airbnb-postgres-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-airbnb-postgres-datamodel', 'e87'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-cars-mysql-datamodel/datamodel-cars',
        component: ComponentCreator('/docs/data-modeling/lab-cars-mysql-datamodel/datamodel-cars', 'ef4'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-cassandra-digital-music-library',
        component: ComponentCreator('/docs/data-modeling/lab-cassandra-digital-music-library', 'cbb'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-cassandra-email-data-model',
        component: ComponentCreator('/docs/data-modeling/lab-cassandra-email-data-model', '9d8'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-cassandra-hotel-reservations',
        component: ComponentCreator('/docs/data-modeling/lab-cassandra-hotel-reservations', 'd8b'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-cassandra-investment-data-model',
        component: ComponentCreator('/docs/data-modeling/lab-cassandra-investment-data-model', '225'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-cassandra-sensor-data-model',
        component: ComponentCreator('/docs/data-modeling/lab-cassandra-sensor-data-model', '714'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-cassandra-shopping-cart-data-model',
        component: ComponentCreator('/docs/data-modeling/lab-cassandra-shopping-cart-data-model', '675'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-disease-diagnosis',
        component: ComponentCreator('/docs/data-modeling/lab-disease-diagnosis', '8b3'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-dvd-rental-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-dvd-rental-datamodel', '883'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-google-playstore-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-google-playstore-datamodel', '854'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-inegi-snowflake-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-inegi-snowflake-datamodel', '086'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-mysql-northwind-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-mysql-northwind-datamodel', 'df8'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-mysql-retail-store-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-mysql-retail-store-datamodel', 'c1a'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-postgres-busrapid-transit',
        component: ComponentCreator('/docs/data-modeling/lab-postgres-busrapid-transit', 'b1d'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-postgres-demographics-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-postgres-demographics-datamodel', '85f'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-postgres-elt-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-postgres-elt-datamodel', 'a97'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-postgres-ewallet-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-postgres-ewallet-datamodel', '49a'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-postgres-housing-cdc-scd',
        component: ComponentCreator('/docs/data-modeling/lab-postgres-housing-cdc-scd', '326'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-sales-datamodel',
        component: ComponentCreator('/docs/data-modeling/lab-sales-datamodel', 'ac2'),
        exact: true
      },
      {
        path: '/docs/data-modeling/lab-sparkify-data-model-postgres',
        component: ComponentCreator('/docs/data-modeling/lab-sparkify-data-model-postgres', '4db'),
        exact: true
      },
      {
        path: '/docs/data-modeling/normalization-vs-denormalization',
        component: ComponentCreator('/docs/data-modeling/normalization-vs-denormalization', 'c6e'),
        exact: true
      },
      {
        path: '/docs/data-modeling/nosql-data-modeling',
        component: ComponentCreator('/docs/data-modeling/nosql-data-modeling', '2b6'),
        exact: true
      },
      {
        path: '/docs/data-modeling/quiz',
        component: ComponentCreator('/docs/data-modeling/quiz', '4fe'),
        exact: true
      },
      {
        path: '/docs/data-modeling/sql-data-modeling',
        component: ComponentCreator('/docs/data-modeling/sql-data-modeling', '23e'),
        exact: true
      },
      {
        path: '/docs/datascience/algorithms/decision-trees',
        component: ComponentCreator('/docs/datascience/algorithms/decision-trees', '69b'),
        exact: true
      },
      {
        path: '/docs/datascience/algorithms/gradient-boosting',
        component: ComponentCreator('/docs/datascience/algorithms/gradient-boosting', '8cd'),
        exact: true
      },
      {
        path: '/docs/datascience/algorithms/knn',
        component: ComponentCreator('/docs/datascience/algorithms/knn', '29c'),
        exact: true
      },
      {
        path: '/docs/datascience/algorithms/linear-regression',
        component: ComponentCreator('/docs/datascience/algorithms/linear-regression', 'db0'),
        exact: true
      },
      {
        path: '/docs/datascience/algorithms/logistic-regression',
        component: ComponentCreator('/docs/datascience/algorithms/logistic-regression', 'c1b'),
        exact: true
      },
      {
        path: '/docs/datascience/algorithms/ml-eda',
        component: ComponentCreator('/docs/datascience/algorithms/ml-eda', '05d'),
        exact: true
      },
      {
        path: '/docs/datascience/algorithms/ml-eda/variable-distribution-analysis',
        component: ComponentCreator('/docs/datascience/algorithms/ml-eda/variable-distribution-analysis', '5af'),
        exact: true
      },
      {
        path: '/docs/datascience/algorithms/random-forest',
        component: ComponentCreator('/docs/datascience/algorithms/random-forest', 'a90'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/cross-domain',
        component: ComponentCreator('/docs/datascience/basics/cross-domain', '876'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/graph-embeddings',
        component: ComponentCreator('/docs/datascience/basics/graph-embeddings', '247'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/graph-networks',
        component: ComponentCreator('/docs/datascience/basics/graph-networks', 'ef0'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/incremental-learning',
        component: ComponentCreator('/docs/datascience/basics/incremental-learning', 'ace'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/meta-learning',
        component: ComponentCreator('/docs/datascience/basics/meta-learning', 'a6e'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/model-retraining',
        component: ComponentCreator('/docs/datascience/basics/model-retraining', '640'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/multi-objective-optimization',
        component: ComponentCreator('/docs/datascience/basics/multi-objective-optimization', '449'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/multitask-learning',
        component: ComponentCreator('/docs/datascience/basics/multitask-learning', 'e45'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/offline-learning',
        component: ComponentCreator('/docs/datascience/basics/offline-learning', 'aad'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/river',
        component: ComponentCreator('/docs/datascience/basics/river', 'fce'),
        exact: true
      },
      {
        path: '/docs/datascience/basics/scalarization',
        component: ComponentCreator('/docs/datascience/basics/scalarization', 'bf4'),
        exact: true
      },
      {
        path: '/docs/datascience/bayesian-optimization',
        component: ComponentCreator('/docs/datascience/bayesian-optimization', '269'),
        exact: true
      },
      {
        path: '/docs/datascience/bias-variance-tradeoff',
        component: ComponentCreator('/docs/datascience/bias-variance-tradeoff', '042'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges',
        component: ComponentCreator('/docs/datascience/challenges', '7b2'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/conversion-rate',
        component: ComponentCreator('/docs/datascience/challenges/conversion-rate', '1e5'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/creditcard-fraud',
        component: ComponentCreator('/docs/datascience/challenges/creditcard-fraud', '94f'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/employee-retention',
        component: ComponentCreator('/docs/datascience/challenges/employee-retention', '03c'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/employee-shuttle-stops',
        component: ComponentCreator('/docs/datascience/challenges/employee-shuttle-stops', 'd49'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/fraud-activity',
        component: ComponentCreator('/docs/datascience/challenges/fraud-activity', 'e34'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/funnel-analysis',
        component: ComponentCreator('/docs/datascience/challenges/funnel-analysis', '646'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/loan-grant',
        component: ComponentCreator('/docs/datascience/challenges/loan-grant', '702'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/marketing-email-campaign',
        component: ComponentCreator('/docs/datascience/challenges/marketing-email-campaign', 'c86'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/price-test',
        component: ComponentCreator('/docs/datascience/challenges/price-test', 'b46'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/translate-ab-test',
        component: ComponentCreator('/docs/datascience/challenges/translate-ab-test', 'd2c'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/Ultimate-data-science-take-home-challenge-sample',
        component: ComponentCreator('/docs/datascience/challenges/Ultimate-data-science-take-home-challenge-sample', '1eb'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/user-referral-program',
        component: ComponentCreator('/docs/datascience/challenges/user-referral-program', '902'),
        exact: true
      },
      {
        path: '/docs/datascience/challenges/video-sharing-analysis',
        component: ComponentCreator('/docs/datascience/challenges/video-sharing-analysis', 'c0f'),
        exact: true
      },
      {
        path: '/docs/datascience/computer-vision',
        component: ComponentCreator('/docs/datascience/computer-vision', 'a1e'),
        exact: true
      },
      {
        path: '/docs/datascience/computer-vision/lab-agri-setallite-image-segmentation',
        component: ComponentCreator('/docs/datascience/computer-vision/lab-agri-setallite-image-segmentation', '0ec'),
        exact: true
      },
      {
        path: '/docs/datascience/computer-vision/lab-image-analytics-tensorflow',
        component: ComponentCreator('/docs/datascience/computer-vision/lab-image-analytics-tensorflow', '0da'),
        exact: true
      },
      {
        path: '/docs/datascience/data-encoding',
        component: ComponentCreator('/docs/datascience/data-encoding', 'ae2'),
        exact: true
      },
      {
        path: '/docs/datascience/data-preparation',
        component: ComponentCreator('/docs/datascience/data-preparation', 'ba1'),
        exact: true
      },
      {
        path: '/docs/datascience/data-splits',
        component: ComponentCreator('/docs/datascience/data-splits', 'f8e'),
        exact: true
      },
      {
        path: '/docs/datascience/deep-learning/deep-learning-basics',
        component: ComponentCreator('/docs/datascience/deep-learning/deep-learning-basics', 'eab'),
        exact: true
      },
      {
        path: '/docs/datascience/deep-learning/perceptron',
        component: ComponentCreator('/docs/datascience/deep-learning/perceptron', 'a06'),
        exact: true
      },
      {
        path: '/docs/datascience/feature-selection',
        component: ComponentCreator('/docs/datascience/feature-selection', 'fde'),
        exact: true
      },
      {
        path: '/docs/datascience/metrics-and-evaluation',
        component: ComponentCreator('/docs/datascience/metrics-and-evaluation', 'f50'),
        exact: true
      },
      {
        path: '/docs/datascience/nlp',
        component: ComponentCreator('/docs/datascience/nlp', '5a2'),
        exact: true
      },
      {
        path: '/docs/datascience/nlp/lab-pdf-wordcloud-mail',
        component: ComponentCreator('/docs/datascience/nlp/lab-pdf-wordcloud-mail', '8cb'),
        exact: true
      },
      {
        path: '/docs/datascience/nlp/text-analysis',
        component: ComponentCreator('/docs/datascience/nlp/text-analysis', '998'),
        exact: true
      },
      {
        path: '/docs/datascience/nlp/text-cleaning',
        component: ComponentCreator('/docs/datascience/nlp/text-cleaning', '0ee'),
        exact: true
      },
      {
        path: '/docs/datascience/nlp/text-processing',
        component: ComponentCreator('/docs/datascience/nlp/text-processing', 'd83'),
        exact: true
      },
      {
        path: '/docs/datascience/nlp/text-style-transfer',
        component: ComponentCreator('/docs/datascience/nlp/text-style-transfer', 'a44'),
        exact: true
      },
      {
        path: '/docs/datascience/nlp/word2vec',
        component: ComponentCreator('/docs/datascience/nlp/word2vec', '9cf'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys',
        component: ComponentCreator('/docs/datascience/recsys', '814'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/amazon-personalize',
        component: ComponentCreator('/docs/datascience/recsys/amazon-personalize', '8ab'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/lab-graph-models',
        component: ComponentCreator('/docs/datascience/recsys/lab-graph-models', '2d8'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/lab-movielens-nmf-pytorch',
        component: ComponentCreator('/docs/datascience/recsys/lab-movielens-nmf-pytorch', '308'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/lab-recsys-amazon-personalize',
        component: ComponentCreator('/docs/datascience/recsys/lab-recsys-amazon-personalize', 'a0a'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/lab-recsys-evaluation',
        component: ComponentCreator('/docs/datascience/recsys/lab-recsys-evaluation', '6d6'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/lab-recsys-matrix-factorizations',
        component: ComponentCreator('/docs/datascience/recsys/lab-recsys-matrix-factorizations', '11d'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/lab-recsys-tourism',
        component: ComponentCreator('/docs/datascience/recsys/lab-recsys-tourism', '82b'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/lab-retail-product-recommend-word2vec',
        component: ComponentCreator('/docs/datascience/recsys/lab-retail-product-recommend-word2vec', 'f91'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/lab-similar-shirt-image-recommender',
        component: ComponentCreator('/docs/datascience/recsys/lab-similar-shirt-image-recommender', '17f'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/most-prominent-architectures',
        component: ComponentCreator('/docs/datascience/recsys/most-prominent-architectures', '296'),
        exact: true
      },
      {
        path: '/docs/datascience/recsys/movielens-mf-node2vec-graph-embeddings',
        component: ComponentCreator('/docs/datascience/recsys/movielens-mf-node2vec-graph-embeddings', '946'),
        exact: true
      },
      {
        path: '/docs/datascience/regression',
        component: ComponentCreator('/docs/datascience/regression', 'bb9'),
        exact: true
      },
      {
        path: '/docs/datascience/reinforcement-learning',
        component: ComponentCreator('/docs/datascience/reinforcement-learning', '053'),
        exact: true
      },
      {
        path: '/docs/datascience/reinforcement-learning/bias-&-fairness',
        component: ComponentCreator('/docs/datascience/reinforcement-learning/bias-&-fairness', '98b'),
        exact: true
      },
      {
        path: '/docs/datascience/reinforcement-learning/causal-inference',
        component: ComponentCreator('/docs/datascience/reinforcement-learning/causal-inference', '121'),
        exact: true
      },
      {
        path: '/docs/datascience/sagemaker',
        component: ComponentCreator('/docs/datascience/sagemaker', 'cd1'),
        exact: true
      },
      {
        path: '/docs/datascience/timeseries',
        component: ComponentCreator('/docs/datascience/timeseries', '8dd'),
        exact: true
      },
      {
        path: '/docs/datascience/timeseries/prophet',
        component: ComponentCreator('/docs/datascience/timeseries/prophet', 'c45'),
        exact: true
      },
      {
        path: '/docs/devops',
        component: ComponentCreator('/docs/devops', '194'),
        exact: true
      },
      {
        path: '/docs/devops/containers',
        component: ComponentCreator('/docs/devops/containers', '64a'),
        exact: true
      },
      {
        path: '/docs/devops/containers/lab-assignment-etl-docker',
        component: ComponentCreator('/docs/devops/containers/lab-assignment-etl-docker', 'c5b'),
        exact: true
      },
      {
        path: '/docs/devops/containers/lab-deploy-simple-docker-ecs',
        component: ComponentCreator('/docs/devops/containers/lab-deploy-simple-docker-ecs', '801'),
        exact: true
      },
      {
        path: '/docs/devops/containers/lab-kubernetes-kubia-app',
        component: ComponentCreator('/docs/devops/containers/lab-kubernetes-kubia-app', 'f14'),
        exact: true
      },
      {
        path: '/docs/devops/fastapi',
        component: ComponentCreator('/docs/devops/fastapi', 'b6b'),
        exact: true
      },
      {
        path: '/docs/devops/fastapi/lab-fastapi-applications',
        component: ComponentCreator('/docs/devops/fastapi/lab-fastapi-applications', 'b43'),
        exact: true
      },
      {
        path: '/docs/devops/fastapi/project-railway-api',
        component: ComponentCreator('/docs/devops/fastapi/project-railway-api', '1d2'),
        exact: true
      },
      {
        path: '/docs/devops/iac',
        component: ComponentCreator('/docs/devops/iac', 'fd1'),
        exact: true
      },
      {
        path: '/docs/extraction',
        component: ComponentCreator('/docs/extraction', '7da'),
        exact: true
      },
      {
        path: '/docs/extraction/api',
        component: ComponentCreator('/docs/extraction/api', '051'),
        exact: true
      },
      {
        path: '/docs/extraction/api/lab-hackernews-git-api',
        component: ComponentCreator('/docs/extraction/api/lab-hackernews-git-api', 'ff2'),
        exact: true
      },
      {
        path: '/docs/extraction/api/lab-scoota',
        component: ComponentCreator('/docs/extraction/api/lab-scoota', 'f69'),
        exact: true
      },
      {
        path: '/docs/extraction/faker',
        component: ComponentCreator('/docs/extraction/faker', 'ccb'),
        exact: true
      },
      {
        path: '/docs/extraction/webscraping',
        component: ComponentCreator('/docs/extraction/webscraping', 'cd1'),
        exact: true
      },
      {
        path: '/docs/extraction/webscraping/lab-finance-extract-load',
        component: ComponentCreator('/docs/extraction/webscraping/lab-finance-extract-load', 'be9'),
        exact: true
      },
      {
        path: '/docs/foundations/basics/25-most-common-interview-questions',
        component: ComponentCreator('/docs/foundations/basics/25-most-common-interview-questions', 'c07'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/50-most-common-interview-questions',
        component: ComponentCreator('/docs/foundations/basics/50-most-common-interview-questions', '587'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/50-most-common-interview-questions-2023',
        component: ComponentCreator('/docs/foundations/basics/50-most-common-interview-questions-2023', 'f8f'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/batch-data-processing',
        component: ComponentCreator('/docs/foundations/basics/batch-data-processing', 'aa7'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/batch-vs-incremental',
        component: ComponentCreator('/docs/foundations/basics/batch-vs-incremental', '726'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/big-data',
        component: ComponentCreator('/docs/foundations/basics/big-data', '27f'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/data-contract',
        component: ComponentCreator('/docs/foundations/basics/data-contract', '56f'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/data-governance',
        component: ComponentCreator('/docs/foundations/basics/data-governance', 'b37'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/data-management',
        component: ComponentCreator('/docs/foundations/basics/data-management', '5ae'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/data-pipelines',
        component: ComponentCreator('/docs/foundations/basics/data-pipelines', '989'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/data-quality',
        component: ComponentCreator('/docs/foundations/basics/data-quality', 'e3b'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/data-storages',
        component: ComponentCreator('/docs/foundations/basics/data-storages', 'a62'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/de-basics',
        component: ComponentCreator('/docs/foundations/basics/de-basics', '638'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/deployment',
        component: ComponentCreator('/docs/foundations/basics/deployment', '9ee'),
        exact: true
      },
      {
        path: '/docs/foundations/basics/extras',
        component: ComponentCreator('/docs/foundations/basics/extras', 'dd6'),
        exact: true
      },
      {
        path: '/docs/foundations/basics/hadoop-basics',
        component: ComponentCreator('/docs/foundations/basics/hadoop-basics', '086'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/hadoop-vs-spark',
        component: ComponentCreator('/docs/foundations/basics/hadoop-vs-spark', '7f6'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/map-reduce',
        component: ComponentCreator('/docs/foundations/basics/map-reduce', 'c6d'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/model-optimization',
        component: ComponentCreator('/docs/foundations/basics/model-optimization', '02a'),
        exact: true
      },
      {
        path: '/docs/foundations/basics/oltp-vs-olap',
        component: ComponentCreator('/docs/foundations/basics/oltp-vs-olap', 'd1a'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/origin',
        component: ComponentCreator('/docs/foundations/basics/origin', 'ed4'),
        exact: true
      },
      {
        path: '/docs/foundations/basics/outliers-handling',
        component: ComponentCreator('/docs/foundations/basics/outliers-handling', '63f'),
        exact: true
      },
      {
        path: '/docs/foundations/basics/rdd',
        component: ComponentCreator('/docs/foundations/basics/rdd', 'd75'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/spark-basics',
        component: ComponentCreator('/docs/foundations/basics/spark-basics', '525'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/spark-dag',
        component: ComponentCreator('/docs/foundations/basics/spark-dag', '534'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/spark-origin',
        component: ComponentCreator('/docs/foundations/basics/spark-origin', '5a4'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/spark-quiz',
        component: ComponentCreator('/docs/foundations/basics/spark-quiz', '7c9'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/sql-vs-nosql',
        component: ComponentCreator('/docs/foundations/basics/sql-vs-nosql', 'a65'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/stream-data-processing',
        component: ComponentCreator('/docs/foundations/basics/stream-data-processing', '7ed'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/basics/use-cases',
        component: ComponentCreator('/docs/foundations/basics/use-cases', 'c15'),
        exact: true
      },
      {
        path: '/docs/foundations/basics/vector-search',
        component: ComponentCreator('/docs/foundations/basics/vector-search', 'b49'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/aws-commands',
        component: ComponentCreator('/docs/foundations/cloud-essentials/aws-commands', '999'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/aws-containers',
        component: ComponentCreator('/docs/foundations/cloud-essentials/aws-containers', '1cd'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/azure-basics',
        component: ComponentCreator('/docs/foundations/cloud-essentials/azure-basics', 'a49'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/azure-batch-processing',
        component: ComponentCreator('/docs/foundations/cloud-essentials/azure-batch-processing', '97f'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/azure-data-ingestion',
        component: ComponentCreator('/docs/foundations/cloud-essentials/azure-data-ingestion', '9de'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/azure-fullstack-solutions',
        component: ComponentCreator('/docs/foundations/cloud-essentials/azure-fullstack-solutions', '23c'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/cloud-basics',
        component: ComponentCreator('/docs/foundations/cloud-essentials/cloud-basics', '688'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/cloud-comparison',
        component: ComponentCreator('/docs/foundations/cloud-essentials/cloud-comparison', 'fc9'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/dms',
        component: ComponentCreator('/docs/foundations/cloud-essentials/dms', '2cd'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/ec2',
        component: ComponentCreator('/docs/foundations/cloud-essentials/ec2', '191'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/gcp-basics',
        component: ComponentCreator('/docs/foundations/cloud-essentials/gcp-basics', 'cfe'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/gcp-setup',
        component: ComponentCreator('/docs/foundations/cloud-essentials/gcp-setup', '41a'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/glue',
        component: ComponentCreator('/docs/foundations/cloud-essentials/glue', '4fb'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/iam',
        component: ComponentCreator('/docs/foundations/cloud-essentials/iam', 'b95'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/lab-aws-secrets-manager',
        component: ComponentCreator('/docs/foundations/cloud-essentials/lab-aws-secrets-manager', '1a7'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/lab-aws-setup',
        component: ComponentCreator('/docs/foundations/cloud-essentials/lab-aws-setup', '31b'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/lab-create-iam-policy-role',
        component: ComponentCreator('/docs/foundations/cloud-essentials/lab-create-iam-policy-role', '135'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/lab-create-your-first-ec2-instance-linux',
        component: ComponentCreator('/docs/foundations/cloud-essentials/lab-create-your-first-ec2-instance-linux', '0f9'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/lab-create-your-first-vpc',
        component: ComponentCreator('/docs/foundations/cloud-essentials/lab-create-your-first-vpc', 'bcf'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/rds',
        component: ComponentCreator('/docs/foundations/cloud-essentials/rds', '4b2'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/s3',
        component: ComponentCreator('/docs/foundations/cloud-essentials/s3', 'fd7'),
        exact: true
      },
      {
        path: '/docs/foundations/cloud-essentials/secrets-manager',
        component: ComponentCreator('/docs/foundations/cloud-essentials/secrets-manager', '469'),
        exact: true
      },
      {
        path: '/docs/foundations/developer/install-anaconda',
        component: ComponentCreator('/docs/foundations/developer/install-anaconda', '937'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/developer/install-dbeaver',
        component: ComponentCreator('/docs/foundations/developer/install-dbeaver', '4df'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/developer/install-jupyter',
        component: ComponentCreator('/docs/foundations/developer/install-jupyter', '741'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/developer/install-vscode',
        component: ComponentCreator('/docs/foundations/developer/install-vscode', 'dde'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/developer/lab-bash-commands',
        component: ComponentCreator('/docs/foundations/developer/lab-bash-commands', '4c7'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/developer/lab-explore-vscode-features',
        component: ComponentCreator('/docs/foundations/developer/lab-explore-vscode-features', '4c7'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/developer/lab-learn-git-commands',
        component: ComponentCreator('/docs/foundations/developer/lab-learn-git-commands', 'a5d'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/developer/setup-git',
        component: ComponentCreator('/docs/foundations/developer/setup-git', '550'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/broadcasting',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/broadcasting', '8e3'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/caching',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/caching', 'b95'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/cheat-sheet',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/cheat-sheet', 'fd9'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/dataframe',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/dataframe', '63e'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/execution-plan',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/execution-plan', '28f'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/install',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/install', '0cb'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lab-bcg',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lab-bcg', 'e83'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lab-pyspark-basics',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lab-pyspark-basics', '6e8'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lab-pyspark-nyctaxi',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lab-pyspark-nyctaxi', 'c6b'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lab-spark-optimizations',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lab-spark-optimizations', 'd91'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lab-spark-optimizations-2',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lab-spark-optimizations-2', '33d'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lab-sql-to-pyspark',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lab-sql-to-pyspark', '792'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lab-uber-analysis',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lab-uber-analysis', 'd88'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lab-understand-spark-query-execution',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lab-understand-spark-query-execution', '5ba'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lab-window-functions',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lab-window-functions', 'bf3'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/lazy-processing',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/lazy-processing', '256'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/methods-operations',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/methods-operations', 'cdd'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/partitioning',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/partitioning', 'b82'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/pyspark-vs-pandas',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/pyspark-vs-pandas', '968'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/pyspark/udf',
        component: ComponentCreator('/docs/foundations/programming-languages/pyspark/udf', '726'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/python',
        component: ComponentCreator('/docs/foundations/programming-languages/python', '50c'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/python/introduction-to-python',
        component: ComponentCreator('/docs/foundations/programming-languages/python/introduction-to-python', 'd28'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/python/links',
        component: ComponentCreator('/docs/foundations/programming-languages/python/links', '0e0'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/python/packaging/docs',
        component: ComponentCreator('/docs/foundations/programming-languages/python/packaging/docs', 'd71'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/python/python-snippets',
        component: ComponentCreator('/docs/foundations/programming-languages/python/python-snippets', '784'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/python/python-syntax-sugars',
        component: ComponentCreator('/docs/foundations/programming-languages/python/python-syntax-sugars', '59d'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/scala',
        component: ComponentCreator('/docs/foundations/programming-languages/scala', 'd0a'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/scala/lab-scala-getting-started',
        component: ComponentCreator('/docs/foundations/programming-languages/scala/lab-scala-getting-started', '981'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/aggregate-functions',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/aggregate-functions', '945'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/and',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/and', '8aa'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/between',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/between', 'a88'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/case',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/case', 'a67'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/assignment1',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/assignment1', '6f4'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/assignment1/solution',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/assignment1/solution', 'cc2'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/assignment2',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/assignment2', '498'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/assignment3',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/assignment3', '1d4'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/assignment4',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/assignment4', '646'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/assignment5',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/assignment5', 'bcc'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/assignment6',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/assignment6', '27e'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/assignment7',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/assignment7', 'f2b'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/braintree',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/braintree', '761'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/employee',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/employee', 'b2e'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/challenges/yammer',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/challenges/yammer', '915'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/comments',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/comments', '4c3'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/comparison-operators',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/comparison-operators', '01c'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/cte',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/cte', '167'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/cursor',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/cursor', 'ace'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/date-functions',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/date-functions', '21b'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/distinct',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/distinct', '024'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/groupby',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/groupby', 'd6b'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/having',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/having', '79d'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/in',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/in', '4dd'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/indexes',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/indexes', 'be6'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/isnull',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/isnull', 'eab'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/joins',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/joins', '7fb'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/lab-mysql-data-ingestion',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/lab-mysql-data-ingestion', '7fe'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/lab-postgres-sales',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/lab-postgres-sales', '256'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/lab-sqlite-basics',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/lab-sqlite-basics', '8ba'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/like',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/like', 'efe'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/limit',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/limit', '654'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/links',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/links', '90e'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/logical-operators',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/logical-operators', '57a'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/not',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/not', '882'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/or',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/or', '786'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/orderby',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/orderby', 'd44'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/performance-tuning',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/performance-tuning', '53a'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/select',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/select', '26d'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/sql-basics',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/sql-basics', '4b4'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/sql-interviews-questions',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/sql-interviews-questions', 'e2c'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/sql-query',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/sql-query', 'cbe'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/stored-procedures',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/stored-procedures', '8fd'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/string-functions',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/string-functions', 'd31'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/subquery',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/subquery', '046'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/triggers',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/triggers', '5b3'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/union',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/union', '3b1'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/views',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/views', 'd0e'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/where',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/where', '9f1'),
        exact: true
      },
      {
        path: '/docs/foundations/programming-languages/sql/window-functions',
        component: ComponentCreator('/docs/foundations/programming-languages/sql/window-functions', '895'),
        exact: true
      },
      {
        path: '/docs/introduction',
        component: ComponentCreator('/docs/introduction', '00f'),
        exact: true,
        sidebar: "docs"
      },
      {
        path: '/docs/mathematics',
        component: ComponentCreator('/docs/mathematics', '54a'),
        exact: true
      },
      {
        path: '/docs/mathematics/probability',
        component: ComponentCreator('/docs/mathematics/probability', '132'),
        exact: true
      },
      {
        path: '/docs/mathematics/probability/probability-distributions',
        component: ComponentCreator('/docs/mathematics/probability/probability-distributions', '732'),
        exact: true
      },
      {
        path: '/docs/mathematics/probability/rules-of-probability',
        component: ComponentCreator('/docs/mathematics/probability/rules-of-probability', '803'),
        exact: true
      },
      {
        path: '/docs/mathematics/statistics',
        component: ComponentCreator('/docs/mathematics/statistics', '829'),
        exact: true
      },
      {
        path: '/docs/mathematics/statistics/sampling',
        component: ComponentCreator('/docs/mathematics/statistics/sampling', '43a'),
        exact: true
      },
      {
        path: '/docs/mlops',
        component: ComponentCreator('/docs/mlops', '9a8'),
        exact: true
      },
      {
        path: '/docs/mlops/code-snippets',
        component: ComponentCreator('/docs/mlops/code-snippets', 'baf'),
        exact: true
      },
      {
        path: '/docs/mlops/ml-lab-tracking',
        component: ComponentCreator('/docs/mlops/ml-lab-tracking', '324'),
        exact: true
      },
      {
        path: '/docs/mlops/mlflow',
        component: ComponentCreator('/docs/mlops/mlflow', '798'),
        exact: true
      },
      {
        path: '/docs/orchestration',
        component: ComponentCreator('/docs/orchestration', '257'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow',
        component: ComponentCreator('/docs/orchestration/airflow', 'b1f'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow/github-nft',
        component: ComponentCreator('/docs/orchestration/airflow/github-nft', '0a1'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow/lab-airflow-conn-py',
        component: ComponentCreator('/docs/orchestration/airflow/lab-airflow-conn-py', '51e'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow/lab-airflow-email-notifications',
        component: ComponentCreator('/docs/orchestration/airflow/lab-airflow-email-notifications', '572'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow/lab-airflow-getting-started',
        component: ComponentCreator('/docs/orchestration/airflow/lab-airflow-getting-started', 'e3d'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow/lab-bike-sharing-service-pipeline',
        component: ComponentCreator('/docs/orchestration/airflow/lab-bike-sharing-service-pipeline', '1f5'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow/lab-forex-etl',
        component: ComponentCreator('/docs/orchestration/airflow/lab-forex-etl', '5dd'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow/lab-imdb-spark-etl',
        component: ComponentCreator('/docs/orchestration/airflow/lab-imdb-spark-etl', '48c'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow/lab-tolldata',
        component: ComponentCreator('/docs/orchestration/airflow/lab-tolldata', 'e04'),
        exact: true
      },
      {
        path: '/docs/orchestration/airflow/labdev-spotify',
        component: ComponentCreator('/docs/orchestration/airflow/labdev-spotify', '5ea'),
        exact: true
      },
      {
        path: '/docs/orchestration/azure-data-factory',
        component: ComponentCreator('/docs/orchestration/azure-data-factory', '453'),
        exact: true
      },
      {
        path: '/docs/orchestration/azure-data-factory/lab-adf-incremental-loading',
        component: ComponentCreator('/docs/orchestration/azure-data-factory/lab-adf-incremental-loading', '386'),
        exact: true
      },
      {
        path: '/docs/orchestration/azure-data-factory/lab-batch-processing-solution',
        component: ComponentCreator('/docs/orchestration/azure-data-factory/lab-batch-processing-solution', '9eb'),
        exact: true
      },
      {
        path: '/docs/orchestration/datafusion',
        component: ComponentCreator('/docs/orchestration/datafusion', 'd93'),
        exact: true
      },
      {
        path: '/docs/orchestration/datafusion/lab-datafusion-pipeline',
        component: ComponentCreator('/docs/orchestration/datafusion/lab-datafusion-pipeline', '95d'),
        exact: true
      },
      {
        path: '/docs/orchestration/modern-data-stack',
        component: ComponentCreator('/docs/orchestration/modern-data-stack', 'daf'),
        exact: true
      },
      {
        path: '/docs/orchestration/modern-data-stack/lab-analyse-stacks',
        component: ComponentCreator('/docs/orchestration/modern-data-stack/lab-analyse-stacks', '691'),
        exact: true
      },
      {
        path: '/docs/orchestration/modern-data-stack/lab-metaflow-snowflake',
        component: ComponentCreator('/docs/orchestration/modern-data-stack/lab-metaflow-snowflake', '6b6'),
        exact: true
      },
      {
        path: '/docs/orchestration/modern-data-stack/lab-terraform-gcp',
        component: ComponentCreator('/docs/orchestration/modern-data-stack/lab-terraform-gcp', 'b6b'),
        exact: true
      },
      {
        path: '/docs/orchestration/nifi',
        component: ComponentCreator('/docs/orchestration/nifi', 'fcf'),
        exact: true
      },
      {
        path: '/docs/orchestration/prefect',
        component: ComponentCreator('/docs/orchestration/prefect', '1ca'),
        exact: true
      },
      {
        path: '/docs/orchestration/prefect/lab-prefect-streaming',
        component: ComponentCreator('/docs/orchestration/prefect/lab-prefect-streaming', '30d'),
        exact: true
      },
      {
        path: '/docs/orchestration/stepfunctions',
        component: ComponentCreator('/docs/orchestration/stepfunctions', 'fcf'),
        exact: true
      },
      {
        path: '/docs/orchestration/stepfunctions/lab-stepfunction-athena-sns',
        component: ComponentCreator('/docs/orchestration/stepfunctions/lab-stepfunction-athena-sns', 'e1a'),
        exact: true
      },
      {
        path: '/docs/orchestration/stepfunctions/lab-stepfunction-ecomm-sqs',
        component: ComponentCreator('/docs/orchestration/stepfunctions/lab-stepfunction-ecomm-sqs', '79b'),
        exact: true
      },
      {
        path: '/docs/processing/apache-beam',
        component: ComponentCreator('/docs/processing/apache-beam', '22a'),
        exact: true
      },
      {
        path: '/docs/processing/apache-druid',
        component: ComponentCreator('/docs/processing/apache-druid', '352'),
        exact: true
      },
      {
        path: '/docs/processing/apache-flink',
        component: ComponentCreator('/docs/processing/apache-flink', '678'),
        exact: true
      },
      {
        path: '/docs/processing/apache-kafka',
        component: ComponentCreator('/docs/processing/apache-kafka', '572'),
        exact: true
      },
      {
        path: '/docs/processing/aws-emr',
        component: ComponentCreator('/docs/processing/aws-emr', 'd45'),
        exact: true
      },
      {
        path: '/docs/processing/aws-kinesis',
        component: ComponentCreator('/docs/processing/aws-kinesis', '0cf'),
        exact: true
      },
      {
        path: '/docs/processing/aws-lambda',
        component: ComponentCreator('/docs/processing/aws-lambda', 'c8f'),
        exact: true
      },
      {
        path: '/docs/processing/aws-lambda-snippets',
        component: ComponentCreator('/docs/processing/aws-lambda-snippets', '868'),
        exact: true
      },
      {
        path: '/docs/processing/azure-synapse-analytics',
        component: ComponentCreator('/docs/processing/azure-synapse-analytics', '694'),
        exact: true
      },
      {
        path: '/docs/processing/databricks',
        component: ComponentCreator('/docs/processing/databricks', 'c5e'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/boston-crime-analysis',
        component: ComponentCreator('/docs/processing/databricks/boston-crime-analysis', '906'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-cybersecurity-databricks',
        component: ComponentCreator('/docs/processing/databricks/lab-cybersecurity-databricks', '429'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-databricks-clickstream',
        component: ComponentCreator('/docs/processing/databricks/lab-databricks-clickstream', 'a87'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-databricks-deltalake',
        component: ComponentCreator('/docs/processing/databricks/lab-databricks-deltalake', '12d'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-databricks-scala-postgres-s3',
        component: ComponentCreator('/docs/processing/databricks/lab-databricks-scala-postgres-s3', '979'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-deltalake-optimizations',
        component: ComponentCreator('/docs/processing/databricks/lab-deltalake-optimizations', '36c'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-dlt-dbt',
        component: ComponentCreator('/docs/processing/databricks/lab-dlt-dbt', 'bcf'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-healthcare-databricks',
        component: ComponentCreator('/docs/processing/databricks/lab-healthcare-databricks', 'bff'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-iot-health-tracker',
        component: ComponentCreator('/docs/processing/databricks/lab-iot-health-tracker', 'a99'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-loan-application',
        component: ComponentCreator('/docs/processing/databricks/lab-loan-application', 'e3a'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-pyspark-itversity',
        component: ComponentCreator('/docs/processing/databricks/lab-pyspark-itversity', 'eb6'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/lab-retail-pos-databricks',
        component: ComponentCreator('/docs/processing/databricks/lab-retail-pos-databricks', '7e6'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/project-advancedbricks',
        component: ComponentCreator('/docs/processing/databricks/project-advancedbricks', 'a66'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/project-bedbricks',
        component: ComponentCreator('/docs/processing/databricks/project-bedbricks', 'cdf'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/project-databricks-de',
        component: ComponentCreator('/docs/processing/databricks/project-databricks-de', '149'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/project-databricks-superset',
        component: ComponentCreator('/docs/processing/databricks/project-databricks-superset', '26b'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/project-learnerbricks',
        component: ComponentCreator('/docs/processing/databricks/project-learnerbricks', '80f'),
        exact: true
      },
      {
        path: '/docs/processing/databricks/setup',
        component: ComponentCreator('/docs/processing/databricks/setup', '61b'),
        exact: true
      },
      {
        path: '/docs/processing/dbt',
        component: ComponentCreator('/docs/processing/dbt', '5e1'),
        exact: true
      },
      {
        path: '/docs/processing/gcp-dataproc',
        component: ComponentCreator('/docs/processing/gcp-dataproc', '17e'),
        exact: true
      },
      {
        path: '/docs/processing/gcp-pubsub',
        component: ComponentCreator('/docs/processing/gcp-pubsub', 'c9f'),
        exact: true
      },
      {
        path: '/docs/processing/lab-azure-hdinsight-simple-data-processing',
        component: ComponentCreator('/docs/processing/lab-azure-hdinsight-simple-data-processing', 'b0e'),
        exact: true
      },
      {
        path: '/docs/processing/lab-azure-synapse-data-processing',
        component: ComponentCreator('/docs/processing/lab-azure-synapse-data-processing', 'd82'),
        exact: true
      },
      {
        path: '/docs/processing/lab-azure-synapse-dataflows',
        component: ComponentCreator('/docs/processing/lab-azure-synapse-dataflows', 'bd8'),
        exact: true
      },
      {
        path: '/docs/processing/lab-azure-synapse-implementing-star-schema',
        component: ComponentCreator('/docs/processing/lab-azure-synapse-implementing-star-schema', 'e38'),
        exact: true
      },
      {
        path: '/docs/processing/lab-confluent-kafka-faker',
        component: ComponentCreator('/docs/processing/lab-confluent-kafka-faker', 'ee2'),
        exact: true
      },
      {
        path: '/docs/processing/lab-csv-to-parquet-conversion',
        component: ComponentCreator('/docs/processing/lab-csv-to-parquet-conversion', 'c55'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dataflow-bigquery-etl',
        component: ComponentCreator('/docs/processing/lab-dataflow-bigquery-etl', '772'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-jaffle-shop',
        component: ComponentCreator('/docs/processing/lab-dbt-jaffle-shop', 'cd4'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-jaffle-shop/models/docs',
        component: ComponentCreator('/docs/processing/lab-dbt-jaffle-shop/models/docs', '773'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-jaffle-shop/models/overview',
        component: ComponentCreator('/docs/processing/lab-dbt-jaffle-shop/models/overview', '3c2'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-knoema',
        component: ComponentCreator('/docs/processing/lab-dbt-knoema', '34b'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-nyctaxi',
        component: ComponentCreator('/docs/processing/lab-dbt-nyctaxi', '756'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-nyctaxi-lookup',
        component: ComponentCreator('/docs/processing/lab-dbt-nyctaxi-lookup', 'c52'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-olist',
        component: ComponentCreator('/docs/processing/lab-dbt-olist', 'a3a'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-olist/models/staging/olist/stg_ecommerce',
        component: ComponentCreator('/docs/processing/lab-dbt-olist/models/staging/olist/stg_ecommerce', '878'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-stackexchnge',
        component: ComponentCreator('/docs/processing/lab-dbt-stackexchnge', '3d9'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-tickit',
        component: ComponentCreator('/docs/processing/lab-dbt-tickit', '49d'),
        exact: true
      },
      {
        path: '/docs/processing/lab-dbt-tpch',
        component: ComponentCreator('/docs/processing/lab-dbt-tpch', '9ae'),
        exact: true
      },
      {
        path: '/docs/processing/lab-emr-serverless',
        component: ComponentCreator('/docs/processing/lab-emr-serverless', '215'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-beam-mapreduce',
        component: ComponentCreator('/docs/processing/lab-gcp-beam-mapreduce', '875'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-dataflow-batch-pipeline',
        component: ComponentCreator('/docs/processing/lab-gcp-dataflow-batch-pipeline', '9f3'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-dataflow-pipeline',
        component: ComponentCreator('/docs/processing/lab-gcp-dataflow-pipeline', '5f0'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-dataflow-side-inputs',
        component: ComponentCreator('/docs/processing/lab-gcp-dataflow-side-inputs', '486'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-dataflow-stream-pipeline',
        component: ComponentCreator('/docs/processing/lab-gcp-dataflow-stream-pipeline', '0c1'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-dataprep',
        component: ComponentCreator('/docs/processing/lab-gcp-dataprep', '7b6'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-dataproc',
        component: ComponentCreator('/docs/processing/lab-gcp-dataproc', 'daa'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-pubsub',
        component: ComponentCreator('/docs/processing/lab-gcp-pubsub', '276'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-pubsub-processing',
        component: ComponentCreator('/docs/processing/lab-gcp-pubsub-processing', 'cc0'),
        exact: true
      },
      {
        path: '/docs/processing/lab-gcp-serverless-dataflow',
        component: ComponentCreator('/docs/processing/lab-gcp-serverless-dataflow', '1d0'),
        exact: true
      },
      {
        path: '/docs/processing/lab-getting-started-with-beam',
        component: ComponentCreator('/docs/processing/lab-getting-started-with-beam', 'c19'),
        exact: true
      },
      {
        path: '/docs/processing/lab-glue-advanced',
        component: ComponentCreator('/docs/processing/lab-glue-advanced', 'eb0'),
        exact: true
      },
      {
        path: '/docs/processing/lab-glue-deltalake-cdc-upsert',
        component: ComponentCreator('/docs/processing/lab-glue-deltalake-cdc-upsert', 'bac'),
        exact: true
      },
      {
        path: '/docs/processing/lab-glue-studio-custom-transforms',
        component: ComponentCreator('/docs/processing/lab-glue-studio-custom-transforms', '2f8'),
        exact: true
      },
      {
        path: '/docs/processing/lab-glue-studio-tickets',
        component: ComponentCreator('/docs/processing/lab-glue-studio-tickets', '82f'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kafka-cli',
        component: ComponentCreator('/docs/processing/lab-kafka-cli', '1de'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kafka-fraud-detection',
        component: ComponentCreator('/docs/processing/lab-kafka-fraud-detection', '017'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kafka-nyctaxi',
        component: ComponentCreator('/docs/processing/lab-kafka-nyctaxi', 'de5'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kafka-nyctaxi/assets',
        component: ComponentCreator('/docs/processing/lab-kafka-nyctaxi/assets', '402'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kafka-python',
        component: ComponentCreator('/docs/processing/lab-kafka-python', '43f'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kafka-spark-streaming',
        component: ComponentCreator('/docs/processing/lab-kafka-spark-streaming', '457'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kafka-stock-market',
        component: ComponentCreator('/docs/processing/lab-kafka-stock-market', 'b08'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kafka-toll-analysis',
        component: ComponentCreator('/docs/processing/lab-kafka-toll-analysis', '47a'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kinesis-apache-logs',
        component: ComponentCreator('/docs/processing/lab-kinesis-apache-logs', '082'),
        exact: true
      },
      {
        path: '/docs/processing/lab-kinesis-clickstream-anomaly',
        component: ComponentCreator('/docs/processing/lab-kinesis-clickstream-anomaly', 'b80'),
        exact: true
      },
      {
        path: '/docs/processing/lab-lambda-csv-parquet',
        component: ComponentCreator('/docs/processing/lab-lambda-csv-parquet', 'fec'),
        exact: true
      },
      {
        path: '/docs/processing/lab-ray-air-basics',
        component: ComponentCreator('/docs/processing/lab-ray-air-basics', '56b'),
        exact: true
      },
      {
        path: '/docs/processing/lab-ray-core-basics',
        component: ComponentCreator('/docs/processing/lab-ray-core-basics', 'cc0'),
        exact: true
      },
      {
        path: '/docs/processing/lab-snowpark-churnpark',
        component: ComponentCreator('/docs/processing/lab-snowpark-churnpark', 'a46'),
        exact: true
      },
      {
        path: '/docs/processing/lab-snowpark-dbtsnowpy',
        component: ComponentCreator('/docs/processing/lab-snowpark-dbtsnowpy', '688'),
        exact: true
      },
      {
        path: '/docs/processing/lab-snowpark-fifapark',
        component: ComponentCreator('/docs/processing/lab-snowpark-fifapark', '5e6'),
        exact: true
      },
      {
        path: '/docs/processing/lab-snowpark-jafflepark',
        component: ComponentCreator('/docs/processing/lab-snowpark-jafflepark', '2a2'),
        exact: true
      },
      {
        path: '/docs/processing/lab-snowpark-jafflepark/models/docs',
        component: ComponentCreator('/docs/processing/lab-snowpark-jafflepark/models/docs', '0d3'),
        exact: true
      },
      {
        path: '/docs/processing/lab-snowpark-knoema-regression',
        component: ComponentCreator('/docs/processing/lab-snowpark-knoema-regression', '881'),
        exact: true
      },
      {
        path: '/docs/processing/project-kafka-ikea',
        component: ComponentCreator('/docs/processing/project-kafka-ikea', 'a58'),
        exact: true
      },
      {
        path: '/docs/processing/ray',
        component: ComponentCreator('/docs/processing/ray', 'd20'),
        exact: true
      },
      {
        path: '/docs/processing/snowpark',
        component: ComponentCreator('/docs/processing/snowpark', 'a92'),
        exact: true
      },
      {
        path: '/docs/storage/apache-couchdb',
        component: ComponentCreator('/docs/storage/apache-couchdb', 'bf6'),
        exact: true
      },
      {
        path: '/docs/storage/apache-hudi',
        component: ComponentCreator('/docs/storage/apache-hudi', '784'),
        exact: true
      },
      {
        path: '/docs/storage/apache-iceberg',
        component: ComponentCreator('/docs/storage/apache-iceberg', 'd99'),
        exact: true
      },
      {
        path: '/docs/storage/arrow',
        component: ComponentCreator('/docs/storage/arrow', '05e'),
        exact: true
      },
      {
        path: '/docs/storage/athena',
        component: ComponentCreator('/docs/storage/athena', '15b'),
        exact: true
      },
      {
        path: '/docs/storage/avro',
        component: ComponentCreator('/docs/storage/avro', 'bfd'),
        exact: true
      },
      {
        path: '/docs/storage/avro-vs-parquet-vs-orc',
        component: ComponentCreator('/docs/storage/avro-vs-parquet-vs-orc', '88c'),
        exact: true
      },
      {
        path: '/docs/storage/aws-dynamodb',
        component: ComponentCreator('/docs/storage/aws-dynamodb', 'ee5'),
        exact: true
      },
      {
        path: '/docs/storage/azure-datalake',
        component: ComponentCreator('/docs/storage/azure-datalake', 'fea'),
        exact: true
      },
      {
        path: '/docs/storage/azure-sql',
        component: ComponentCreator('/docs/storage/azure-sql', '343'),
        exact: true
      },
      {
        path: '/docs/storage/bigquery',
        component: ComponentCreator('/docs/storage/bigquery', '3cb'),
        exact: true
      },
      {
        path: '/docs/storage/casestudy-messflix-hypothetical',
        component: ComponentCreator('/docs/storage/casestudy-messflix-hypothetical', 'd9c'),
        exact: true
      },
      {
        path: '/docs/storage/cassandra',
        component: ComponentCreator('/docs/storage/cassandra', '458'),
        exact: true
      },
      {
        path: '/docs/storage/code-snippets',
        component: ComponentCreator('/docs/storage/code-snippets', '9ba'),
        exact: true
      },
      {
        path: '/docs/storage/compression',
        component: ComponentCreator('/docs/storage/compression', 'd10'),
        exact: true
      },
      {
        path: '/docs/storage/csv',
        component: ComponentCreator('/docs/storage/csv', '968'),
        exact: true
      },
      {
        path: '/docs/storage/data-lakehouses',
        component: ComponentCreator('/docs/storage/data-lakehouses', '5bb'),
        exact: true
      },
      {
        path: '/docs/storage/data-mesh-basics',
        component: ComponentCreator('/docs/storage/data-mesh-basics', '6fc'),
        exact: true
      },
      {
        path: '/docs/storage/data-warehouses',
        component: ComponentCreator('/docs/storage/data-warehouses', '462'),
        exact: true
      },
      {
        path: '/docs/storage/database-storage-engines',
        component: ComponentCreator('/docs/storage/database-storage-engines', 'c15'),
        exact: true
      },
      {
        path: '/docs/storage/datalake-components',
        component: ComponentCreator('/docs/storage/datalake-components', '089'),
        exact: true
      },
      {
        path: '/docs/storage/datalakes',
        component: ComponentCreator('/docs/storage/datalakes', 'feb'),
        exact: true
      },
      {
        path: '/docs/storage/deltalake',
        component: ComponentCreator('/docs/storage/deltalake', '357'),
        exact: true
      },
      {
        path: '/docs/storage/duckdb',
        component: ComponentCreator('/docs/storage/duckdb', '9da'),
        exact: true
      },
      {
        path: '/docs/storage/gcp-bigtable',
        component: ComponentCreator('/docs/storage/gcp-bigtable', '3ae'),
        exact: true
      },
      {
        path: '/docs/storage/gcp-cloudsql',
        component: ComponentCreator('/docs/storage/gcp-cloudsql', '53e'),
        exact: true
      },
      {
        path: '/docs/storage/gcs',
        component: ComponentCreator('/docs/storage/gcs', 'c7d'),
        exact: true
      },
      {
        path: '/docs/storage/hadoop-vs-datalake',
        component: ComponentCreator('/docs/storage/hadoop-vs-datalake', '103'),
        exact: true
      },
      {
        path: '/docs/storage/hudi',
        component: ComponentCreator('/docs/storage/hudi', '5c3'),
        exact: true
      },
      {
        path: '/docs/storage/iceberg',
        component: ComponentCreator('/docs/storage/iceberg', '5bf'),
        exact: true
      },
      {
        path: '/docs/storage/interview-questions',
        component: ComponentCreator('/docs/storage/interview-questions', 'a66'),
        exact: true
      },
      {
        path: '/docs/storage/json',
        component: ComponentCreator('/docs/storage/json', '928'),
        exact: true
      },
      {
        path: '/docs/storage/lab-adl-securing-monitoring-lakes',
        component: ComponentCreator('/docs/storage/lab-adl-securing-monitoring-lakes', '5af'),
        exact: true
      },
      {
        path: '/docs/storage/lab-amazon-keyspaces',
        component: ComponentCreator('/docs/storage/lab-amazon-keyspaces', '20f'),
        exact: true
      },
      {
        path: '/docs/storage/lab-aws-rds-service',
        component: ComponentCreator('/docs/storage/lab-aws-rds-service', '685'),
        exact: true
      },
      {
        path: '/docs/storage/lab-azure-sql-securing-databases',
        component: ComponentCreator('/docs/storage/lab-azure-sql-securing-databases', '070'),
        exact: true
      },
      {
        path: '/docs/storage/lab-bigquery-analysis',
        component: ComponentCreator('/docs/storage/lab-bigquery-analysis', '674'),
        exact: true
      },
      {
        path: '/docs/storage/lab-bigquery-commandline',
        component: ComponentCreator('/docs/storage/lab-bigquery-commandline', '629'),
        exact: true
      },
      {
        path: '/docs/storage/lab-bigquery-composer',
        component: ComponentCreator('/docs/storage/lab-bigquery-composer', '1c2'),
        exact: true
      },
      {
        path: '/docs/storage/lab-bigquery-data-warehousing',
        component: ComponentCreator('/docs/storage/lab-bigquery-data-warehousing', '8d7'),
        exact: true
      },
      {
        path: '/docs/storage/lab-bigquery-ml',
        component: ComponentCreator('/docs/storage/lab-bigquery-ml', '575'),
        exact: true
      },
      {
        path: '/docs/storage/lab-bigquery-nyctaxi',
        component: ComponentCreator('/docs/storage/lab-bigquery-nyctaxi', '97d'),
        exact: true
      },
      {
        path: '/docs/storage/lab-bigquery-optimization',
        component: ComponentCreator('/docs/storage/lab-bigquery-optimization', '188'),
        exact: true
      },
      {
        path: '/docs/storage/lab-bigquery-query-optimization',
        component: ComponentCreator('/docs/storage/lab-bigquery-query-optimization', 'd19'),
        exact: true
      },
      {
        path: '/docs/storage/lab-biqeury-building-warehouse',
        component: ComponentCreator('/docs/storage/lab-biqeury-building-warehouse', '49b'),
        exact: true
      },
      {
        path: '/docs/storage/lab-couchdb-movies-data-migration',
        component: ComponentCreator('/docs/storage/lab-couchdb-movies-data-migration', '0fd'),
        exact: true
      },
      {
        path: '/docs/storage/lab-data-loading-python',
        component: ComponentCreator('/docs/storage/lab-data-loading-python', '976'),
        exact: true
      },
      {
        path: '/docs/storage/lab-datalake-healthcare-s3-glue-athena',
        component: ComponentCreator('/docs/storage/lab-datalake-healthcare-s3-glue-athena', '84f'),
        exact: true
      },
      {
        path: '/docs/storage/lab-db2-bookshop-petsale-data-ingestion',
        component: ComponentCreator('/docs/storage/lab-db2-bookshop-petsale-data-ingestion', '1b7'),
        exact: true
      },
      {
        path: '/docs/storage/lab-gcp-cloudsql-nyctaxi',
        component: ComponentCreator('/docs/storage/lab-gcp-cloudsql-nyctaxi', '7fb'),
        exact: true
      },
      {
        path: '/docs/storage/lab-gcp-streaming-bigtable',
        component: ComponentCreator('/docs/storage/lab-gcp-streaming-bigtable', '57c'),
        exact: true
      },
      {
        path: '/docs/storage/lab-getting-started-with-cassandra/cassandra-cli',
        component: ComponentCreator('/docs/storage/lab-getting-started-with-cassandra/cassandra-cli', 'be7'),
        exact: true
      },
      {
        path: '/docs/storage/lab-getting-started-with-cassandra/getting-started',
        component: ComponentCreator('/docs/storage/lab-getting-started-with-cassandra/getting-started', '409'),
        exact: true
      },
      {
        path: '/docs/storage/lab-glue-emr-iceberg-serverless-lakehouse',
        component: ComponentCreator('/docs/storage/lab-glue-emr-iceberg-serverless-lakehouse', 'fff'),
        exact: true
      },
      {
        path: '/docs/storage/lab-intro-to-dynamodb',
        component: ComponentCreator('/docs/storage/lab-intro-to-dynamodb', '897'),
        exact: true
      },
      {
        path: '/docs/storage/lab-mongodb-basics',
        component: ComponentCreator('/docs/storage/lab-mongodb-basics', 'f41'),
        exact: true
      },
      {
        path: '/docs/storage/lab-mongodb-pandas',
        component: ComponentCreator('/docs/storage/lab-mongodb-pandas', 'a29'),
        exact: true
      },
      {
        path: '/docs/storage/lab-mysql-data-ingestion',
        component: ComponentCreator('/docs/storage/lab-mysql-data-ingestion', '96a'),
        exact: true
      },
      {
        path: '/docs/storage/lab-mysql-police-api-etl',
        component: ComponentCreator('/docs/storage/lab-mysql-police-api-etl', 'b35'),
        exact: true
      },
      {
        path: '/docs/storage/lab-postgres-bash-etl',
        component: ComponentCreator('/docs/storage/lab-postgres-bash-etl', 'ec4'),
        exact: true
      },
      {
        path: '/docs/storage/lab-postgres-crime-reports',
        component: ComponentCreator('/docs/storage/lab-postgres-crime-reports', 'c7c'),
        exact: true
      },
      {
        path: '/docs/storage/lab-postgres-getting-started',
        component: ComponentCreator('/docs/storage/lab-postgres-getting-started', '4c6'),
        exact: true
      },
      {
        path: '/docs/storage/lab-postgres-trips-stored-procedure',
        component: ComponentCreator('/docs/storage/lab-postgres-trips-stored-procedure', 'b3c'),
        exact: true
      },
      {
        path: '/docs/storage/lab-processing-json-data',
        component: ComponentCreator('/docs/storage/lab-processing-json-data', '31f'),
        exact: true
      },
      {
        path: '/docs/storage/lab-production-cleaning-deltalake',
        component: ComponentCreator('/docs/storage/lab-production-cleaning-deltalake', '86a'),
        exact: true
      },
      {
        path: '/docs/storage/lab-read-s3-delta-in-python',
        component: ComponentCreator('/docs/storage/lab-read-s3-delta-in-python', '5c7'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-data-loading',
        component: ComponentCreator('/docs/storage/lab-redshift-data-loading', 'c7a'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-data-loading-analysis',
        component: ComponentCreator('/docs/storage/lab-redshift-data-loading-analysis', '1de'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-data-loading-python',
        component: ComponentCreator('/docs/storage/lab-redshift-data-loading-python', '103'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-immersion',
        component: ComponentCreator('/docs/storage/lab-redshift-immersion', '9e5'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-ml',
        component: ComponentCreator('/docs/storage/lab-redshift-ml', '6ef'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-ongoing-load-elt',
        component: ComponentCreator('/docs/storage/lab-redshift-ongoing-load-elt', '9ac'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-scd',
        component: ComponentCreator('/docs/storage/lab-redshift-scd', 'a7a'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-scd-2',
        component: ComponentCreator('/docs/storage/lab-redshift-scd-2', '32f'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-spectrum-query-datalake',
        component: ComponentCreator('/docs/storage/lab-redshift-spectrum-query-datalake', 'a7c'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-spectrum-query-tuning',
        component: ComponentCreator('/docs/storage/lab-redshift-spectrum-query-tuning', 'ab8'),
        exact: true
      },
      {
        path: '/docs/storage/lab-redshift-table-design-query-tuning',
        component: ComponentCreator('/docs/storage/lab-redshift-table-design-query-tuning', 'bd8'),
        exact: true
      },
      {
        path: '/docs/storage/lab-scd-glue-delta',
        component: ComponentCreator('/docs/storage/lab-scd-glue-delta', '67d'),
        exact: true
      },
      {
        path: '/docs/storage/lab-snowflake-getting-started',
        component: ComponentCreator('/docs/storage/lab-snowflake-getting-started', '434'),
        exact: true
      },
      {
        path: '/docs/storage/lab-snowflake-snowsql',
        component: ComponentCreator('/docs/storage/lab-snowflake-snowsql', 'c12'),
        exact: true
      },
      {
        path: '/docs/storage/lab-snowpark-churn',
        component: ComponentCreator('/docs/storage/lab-snowpark-churn', '7f6'),
        exact: true
      },
      {
        path: '/docs/storage/lab-sqlite-basics',
        component: ComponentCreator('/docs/storage/lab-sqlite-basics', 'a4f'),
        exact: true
      },
      {
        path: '/docs/storage/lab-sqlite-hipolabs-api',
        component: ComponentCreator('/docs/storage/lab-sqlite-hipolabs-api', 'bc5'),
        exact: true
      },
      {
        path: '/docs/storage/lab-zero-to-snowflake',
        component: ComponentCreator('/docs/storage/lab-zero-to-snowflake', '76a'),
        exact: true
      },
      {
        path: '/docs/storage/mongodb',
        component: ComponentCreator('/docs/storage/mongodb', '734'),
        exact: true
      },
      {
        path: '/docs/storage/mysql',
        component: ComponentCreator('/docs/storage/mysql', '845'),
        exact: true
      },
      {
        path: '/docs/storage/orc',
        component: ComponentCreator('/docs/storage/orc', '9b0'),
        exact: true
      },
      {
        path: '/docs/storage/parquet',
        component: ComponentCreator('/docs/storage/parquet', 'da0'),
        exact: true
      },
      {
        path: '/docs/storage/parquet-vs-csv',
        component: ComponentCreator('/docs/storage/parquet-vs-csv', '047'),
        exact: true
      },
      {
        path: '/docs/storage/postgres',
        component: ComponentCreator('/docs/storage/postgres', '2f4'),
        exact: true
      },
      {
        path: '/docs/storage/postgres-vs-mysql',
        component: ComponentCreator('/docs/storage/postgres-vs-mysql', '13f'),
        exact: true
      },
      {
        path: '/docs/storage/project-athena-federated',
        component: ComponentCreator('/docs/storage/project-athena-federated', '9aa'),
        exact: true
      },
      {
        path: '/docs/storage/redshift',
        component: ComponentCreator('/docs/storage/redshift', 'bf4'),
        exact: true
      },
      {
        path: '/docs/storage/serialization',
        component: ComponentCreator('/docs/storage/serialization', '52c'),
        exact: true
      },
      {
        path: '/docs/storage/snowflake',
        component: ComponentCreator('/docs/storage/snowflake', 'a01'),
        exact: true
      },
      {
        path: '/docs/storage/sqlite',
        component: ComponentCreator('/docs/storage/sqlite', '140'),
        exact: true
      },
      {
        path: '/docs/storage/why-datalakes',
        component: ComponentCreator('/docs/storage/why-datalakes', '1fa'),
        exact: true
      },
      {
        path: '/docs/storage/xml',
        component: ComponentCreator('/docs/storage/xml', '63e'),
        exact: true
      },
      {
        path: '/docs/visualization',
        component: ComponentCreator('/docs/visualization', '67b'),
        exact: true
      },
      {
        path: '/docs/visualization/flask',
        component: ComponentCreator('/docs/visualization/flask', 'b82'),
        exact: true
      },
      {
        path: '/docs/visualization/looker-studio',
        component: ComponentCreator('/docs/visualization/looker-studio', 'ca9'),
        exact: true
      },
      {
        path: '/docs/visualization/looker-studio/lab-gcp-streaming-analytics',
        component: ComponentCreator('/docs/visualization/looker-studio/lab-gcp-streaming-analytics', 'f60'),
        exact: true
      },
      {
        path: '/docs/visualization/preset',
        component: ComponentCreator('/docs/visualization/preset', '7c7'),
        exact: true
      },
      {
        path: '/docs/visualization/streamlit',
        component: ComponentCreator('/docs/visualization/streamlit', '529'),
        exact: true
      }
    ]
  },
  {
    path: '/',
    component: ComponentCreator('/', '1a8'),
    exact: true
  },
  {
    path: '*',
    component: ComponentCreator('*'),
  },
];
